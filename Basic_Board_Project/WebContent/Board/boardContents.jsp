...
<head>
      ...
      $.each(commentsList, function (index, comment) {
         ...
         if(<c:out value="${not empty sessionLoginUser}"/>){
            goodbad.append($("<input>").attr({
               type: "button",
               onclick: "createSubComment(" + comment.COMMENT_CODE + ",'" + comment.NAME + "');",
               value: "답글"
            }));
         }

         // 대댓글 달기 폼
         var subCommentForm = commentDiv.find("#subCommentForm"+comment.COMMENT_CODE);
         subCommentForm.append($("<form>").attr({
            action: "/Basic_Board_Project/BoardController?action=insertSubComment",
            method: "POST",
            id : "subCommentSubmit" + comment.COMMENT_CODE
         }));
         ...
         formDiv.append($("<input>").attr({
            type: "button",
            value: "댓글달기",
            onclick: "insertSubComment(" + comment.COMMENT_CODE + ");",
            "class" : "btn btn-dark",
         }));
         // 임시 콘솔 출력
         console.log(
   ...
   function createSubComment(COMMENT_CODE, NAME) {
      var subCommentForm = document.getElementById("subCommentForm" + COMMENT_CODE);
      subCommentForm.setAttribute("style", "display:block; padding-left: 50px;");

       //"@원댓글작성자" 표시
      $("#subCommentForm"+COMMENT_CODE).find("form").find("textarea").text("@" + NAME + " ");  
   }
